...
import finish from '../../../../assets/images/Auditions/finish.JPG'
import swal from 'sweetalert';
import LearningSessionNaveStar from "./LearnSessionNaveStar";


const PendingLearningSessionDetails = (props) => {
        ...
        axios.get('/sanctum/csrf-cookie').then(response => {
            axios.get(`/api/star/approve_learning_session/${event.id}`).then(res => {
                    if(res.data.status === 200)
                    {
                      ...
                      history.push('/superstar/learning-session');
                    }
            });
        });
  ...
  return (
    <>
      <div className="AS">
        <LearningSessionNaveStar></LearningSessionNaveStar>
...
export default PendingLearningSessionDetails;
