...
const PendingProductList = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/admin/pending/auction/product`).then((res) => {
          ...
          if (res.status === 200) {
            console.log('from api',res.data.pending_product)
            setPendingProduct(res.data.pending_product);
            ...
            <div className="SouvenirI mx-4 mt-3">
                ...
                <div className="mb-4">
                    <div className="text-light fw-bold mt-1 mx-2">
                        <Link to={`/superstar-admin/product/details/${product.id}`} style={{ textDecoration: 'none', color:'white' }}>
                        <h3>{product.name}</h3>
                        </Link>
